...
function testWithImplementation(prefix, Observable) {
	...
	// RxJs does not honor this contract - resolves to null.
	if (Observable === ZenObservable) {
		...
		test(`${prefix}: forEach resolves with the value of the awaited promise`, async t => {
			t.plan(3);
			var ee = new EventEmitter();
			...
			var result = await m(ee, {endEvent: false, await: awaited})
				.forEach(chunk => t.is(chunk, expected.next().value));
			t.is(result, 'resolution');
		});
	}
